...
use Countable;
use Exception;
use StudentLink\DataCorruptedException;
use StudentLink\EntityNotFoundException;
use StudentLink\InvalidInputException;
use StudentLink\InvalidPropertyValueException;
...
abstract class Entity implements Countable
{
    ...
    private function runDynamicValidationIfExists($property, $value, $propDef)
    {
        ...
        $data = $this->data;
        try {
            $result = eval("use \StudentLink\InvalidPropertyValueException;\r\n" . $code);
            if (isset($result)) {
