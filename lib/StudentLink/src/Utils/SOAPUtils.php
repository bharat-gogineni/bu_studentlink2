...
class SOAPUtils
{
    ...
    public function makeCall(string $function, array $data, bool $clean = true)
    {
        if ($errors = $this->getValidData($function, $data)) {
            throw new ServiceException(json_encode($errors), 'validation.errors', StudentLinkServiceException::ERR_CODE_NOT_ACCEPTABLE);
        }
        try {
            $response = $this->client->{$function}($data);
        } catch (Exception $e) {
            throw new ServiceException($e->getMessage(), 'soap.call.errors', StudentLinkServiceException::ERR_CODE_INTERNAL_SERVER_ERROR);
        }
        if ($clean) {
            ...
            if (isset($data[$key]) && isset($value['type'])) {
                switch ($value['type']) {
                    ...
                    case 'pattern':
                        ...
                        break;
                    default:
                        throw new ServiceException("Unable to validate ".json_encode($value)." = ".$data[$key], 'validation.errors', StudentLinkServiceException::ERR_CODE_NOT_FOUND);
                        break;
                }
            }
    ...
    public function getFunctionStruct(string $function)
    {
        if (!$this->isValidFunction($function)) {
            throw new ServiceException("Requested function not found", 'function.not.found', StudentLinkServiceException::ERR_CODE_NOT_FOUND);
        }
        ...
        if (count($schemas) != 1) {
            throw new ServiceException("Schema not found", 'schema.not.found', StudentLinkServiceException::ERR_CODE_NOT_FOUND);
        }
    ...
    private function processElementType($element)
    {
        ...
        foreach ($elementTypes as $elementType) {
            ...
            if (count($elementTypeDom) > 1) {
                throw new ServiceException("More than one element found", 'schema.error', StudentLinkServiceException::ERR_CODE_CONFLICT);
            ...
            switch ($elementType) {
                ...
                case 'simpleType':
                    ...
                    break;
                default:
                    throw new ServiceException("Unknown element type", 'schema.error', StudentLinkServiceException::ERR_CODE_UNPROCESSABLE_ENTITY);
                    break;
            }
        }
    }
            ...
            foreach ($value->children(current($value->getNamespaces())) as $element) {
                switch ($element->getName()) {
                    ...
                    case 'pattern':
                        ...
                        break;
                    default:
                        throw new ServiceException("Unknown simple element type", 'schema.error', StudentLinkServiceException::ERR_CODE_UNPROCESSABLE_ENTITY);
                        break;
                }
            }
        }
