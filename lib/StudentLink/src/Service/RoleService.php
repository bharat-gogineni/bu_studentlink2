...
use StudentLink\Model\PrivilegeTable;
use StudentLink\Model\Role;
...
class RoleService extends AbstractService
{
    ...
    public function deleteRole($id, $params)
    {
        ...
        if (is_null($obj)) {
            throw new ServiceException("Role not found", "role.not.found", StudentLinkServiceException::ERR_CODE_NOT_FOUND);
        }

        if (isset($accountId)) {
            if ($accountId != $obj->account_id) {
                throw new ServiceException("Role does not belong to the account", "role.not.found", StudentLinkServiceException::ERR_CODE_NOT_FOUND);
            }
        }
        if ($obj->default_role == 1) {
            throw new ServiceException("System Roles cannot be deleted", "role.cannot.delete", StudentLinkServiceException::ERR_CODE_NOT_FOUND);
        }
    ...
    public function getRole($params)
    {
        ...
        if (!$roleId) {
            throw new ServiceException("Invalid Role", 'invalid.role', StudentLinkServiceException::ERR_CODE_NOT_FOUND);
        }
