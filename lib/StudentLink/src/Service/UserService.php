...
use Exception;
use StudentLink\AccessDeniedException;
...
use StudentLink\Model\Account;
use StudentLink\ServiceException;
use StudentLink\StudentLinkServiceException;
use StudentLink\InsertFailedException;
use StudentLink\EntityNotFoundException;
use StudentLink\Utils\BosUtils;
use StudentLink\Utils\FilterUtils;
use StudentLink\Utils\UuidUtil;
...
class UserService extends AbstractService
{
    ...
    public function __construct($config, $dbAdapter, UserTable $table = null)
    {
    ...
    public function getUserContextDetails($userName)
    {
        ...
        $select = "SELECT * from users as ou where ou.username = :userName";
    ...
    public function getTeamsFromDb($id)
    {
        ...
        $select = $sql->select()
            ...
            ->join('users_team', 'users_team.team_id = ox_team.id', array())
            ->where(array('users_team.avatar_id' => $id));
    ...
    public function createUser($params, &$userData, $register = false)
    {
         ...
         */
        if (count($result) > 1) {
            throw new ServiceException("Username or Email Exists in other Account", "user.email.exists", StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED);
        }
        if (count($result) == 1) {
            ...
            if (in_array($data['account_id'], $accountList)) {
                ...
                if ($result[0]['username'] == $data['username'] && $result[0]['status'] == 'Active') {
                    throw new ServiceException("Username/Email Exists", "duplicate.username", StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED);
                } elseif ($result[0]['email'] == $data['email'] && $result[0]['status'] == 'Active') {
                    throw new ServiceException("Email Exists", "duplicate.email", StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED);
                } elseif ($result[0]['status'] == "Inactive") {
                    $data['reactivate'] = isset($data['reactivate']) ? $data['reactivate'] : 0;
                    if ($data['reactivate'] == 0) {
                        throw new ServiceException("user already exists and is inactive. Please contact the admin to activate", "user.already.exists", StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED);
                    }
                    $this->reactivateUserAccount($result[0]['id'], $data);
                    return $result[0]['uuid'];
                }
            } else {
                throw new InsertFailedException("Username or Email Exists in other Account", StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED);
            }
        }
        try {
    ...
    private function updateUserProjects($userId, $project, $accountId)
    {
        ...
        $delete = "DELETE oxup FROM users_project as oxup
        ...
        $query = "INSERT into users_project(user_id,project_id) 
                    SELECT :userId,oxp.id from ox_project as oxp 
                    inner join ox_account acct on acct.id = oxp.account_id
                    LEFT OUTER JOIN users_project as oxup on oxp.id = oxup.project_id 
    ...
    private function addRoleToUser($accountUserId, $role, $accountId)
    {
        ...
        $params = ['accountUserId' => $accountUserId,
                    'accountId' => $accountId];
        $delete = "DELETE our FROM users_role as our
        ...
        $query = "INSERT into users_role(account_user_id,role_id) 
                    SELECT :accountUserId, oro.id 
                    from ox_role as oro 
                    LEFT OUTER JOIN users_role as our on oro.id = our.role_id 
    ...
    private function addUserRole($accountUserId, $roleName, $appId = null)
    {
        if (!is_numeric($accountUserId)) {
            throw new ServiceException('Invalid Parameter passed', 'invalid.parameter', StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED);
        }
        ...
        if ($user) {
            ...
            if (!empty($role)) {
                if (!$this->getDataByParams('users_role', array(), array('account_user_id' => $user[0]['id'], 'role_id' => $role[0]['id']))->toArray()) {
                    $data = array(array(
                        'account_user_id' => $user[0]['id'],
                        'role_id' => $role[0]['id'],
                    ));
                    $this->logger->info("Executing Data---".print_r($data, true));
                    $result = $this->multiInsertOrUpdate('users_role', $data);
    ...
    public function updateUser($id, &$data, $accountId = null)
    {
        ...
        if (isset($accountId)) {
            ...
            if (!in_array($accountId, $acctArray)) {
                throw new ServiceException('User does not belong to the Account', 'user.not.found', StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED);
            }
        }
    ...
    public function deleteUser($id)
    {
        ...
        if ($result1[0]['contactid'] == $form->id) {
            throw new ServiceException('Not allowed to delete Admin user', 'admin.user', StudentLinkServiceException::ERR_CODE_FORBIDDEN);
        }
        ...
        if ($result2[0]['count(id)'] > 0) {
            throw new ServiceException('Not allowed to delete the team manager', 'team.manager', StudentLinkServiceException::ERR_CODE_FORBIDDEN);
        }
        ...
        if ($result3[0]['count(id)'] > 0) {
            throw new ServiceException('Not allowed to delete the project manager', 'project.manager', StudentLinkServiceException::ERR_CODE_FORBIDDEN);
        }
    ...
    {
        $uuidList = array_unique(array_map('current', $uuidList));
        $query = "SELECT id from users where uuid in ('" . implode("','", $uuidList) . "')";
        $result = $this->executeQueryWithParams($query)->toArray();
        return $result;
    }
    ...
    public function getUsers($filterParams = null, $baseUrl = '', $params = null)
    {
        ...
        $from = " FROM `users` as ou 
                  ...
                  LEFT JOIN users manager_user on manager_user.person_id = man.person_id
    ...
    {
        $id = (is_numeric($id)) ? $id : $this->getIdFromUuid('users', $id);
        $select = "SELECT ou.uuid,ou.username,per.firstname,per.lastname,ou.name,
                    ...
                    left join users as manager_user on manager_user.person_id = man.person_id
    ...
    public function hasAccount(&$accountId, $userId = null)
    {
        ...
        if ($userId === null) {
            ...
            $userId = $this->getIdFromUuid('users', $userId);
        }
    ...
    private function getPrivilegesFromDb($userId, $accountId)
    {
        $query = "SELECT privilege_name, permission 
                    ...
                    INNER join users_role ur on ur.role_id = rp.role_id
    ...
    {
        $select = "SELECT * from users as ou where ou.id = :userId ";
        $params = [ 'userId' => $id];
        $response = $this->executeQueryWithBindParameters($select, $params)->toArray();
        if (empty($response)) {
    ...
    private function addUserToProject($userId, $projectList)
    {
        ...
        $this->multiInsertOrUpdate('users_project', $oxUserProject, array());
    }

    public function removeUserFromTeam($userid)
    {
        $sql = $this->getSqlObject();
        $queryString = "select avatar_id from users_team";
    ...
    private function addUserToAccount($userId, $accountId)
    {
        ...
        $user = $this->getDataByParams('users', array('id', 'username'), array('id' => $userId))->toArray();
        if (empty($user)) {
    ...
    /**
     * @return \StudentLink\Utils\Array
     */
        ...
        $query = "SELECT uuid, name
                    FROM (
                        ...
                        LEFT JOIN(
                            ...
                            INNER JOIN users_role AS ou ON orp.role_id = ou.role_id
    ...
    public function resetPassword($data)
    {
        ...
        $query = "UPDATE users set password = :password, password_reset_code = NULL, password_reset_expiry_date = NULL where password_reset_code = :resetCode";
        $this->executeUpdateWithBindParameters($query, $params);
    }
    ...
    public function sendResetPasswordCode($username)
    {
        ...
        throw new ServiceException("Password reset failed", 'password.reset.failed', StudentLinkServiceException::ERR_CODE_UNPROCESSABLE_ENTITY);
    }
    ...
    public function getOrganizationByUserId($id = null)
    {
        ...
        $query = "SELECT acct.name,acct.uuid as accountId, oxa.address1,oxa.address2,oxa.city,
                    ...
                    LEFT JOIN users as u on u.id = uo.user_id
    ...
    public function getAppsByUserId($id = null)
    {
        ...
        $query = "SELECT * from 
                  (SELECT DISTINCT oa.name,oa.description, oa.uuid, oa.type, oa.logo, oa.category 
                    ...
                    INNER JOIN users_role as our ON orp.role_id = our.role_id AND 
    ...
    FROM
        ...
        LEFT JOIN users manUser on manUser.person_id = man.person_id
    ...
    public function checkUserExists($data)
    {
        ...
        if (!empty($result)) {
            if ($data['username'] == $result[0]['username'] &&
                ...
                throw new ServiceException("Username/Email Used", "username.exists", StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED);
            }
        }
        return 0;
    }
    ...
    public function getUsersList($appUUid, $params)
    {
        try {
            ...
            if (!empty($result)) {
                ...
                throw new ServiceException("App Does not belong to the account", "app.for.account.not.found", StudentLinkServiceException::ERR_CODE_FORBIDDEN);
            }
        } catch (Exception $e) {
            throw $e;
        }
    ...
    {
        $select = "SELECT ou.* from users as ou 
    ...
    public function updateLoggedInStatus()
    {
        ...
        $update = $sql->update('users')->set($updatedData)
        ->where(array('users.id' => AuthContext::get(AuthConstants::USER_ID),'users.account_id' => AuthContext::get(AuthConstants::ACCOUNT_ID)));
