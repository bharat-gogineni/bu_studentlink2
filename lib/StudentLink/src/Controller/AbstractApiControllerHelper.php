...
namespace StudentLink\Controller;

use Logger;
use StudentLink\Error\ErrorHandler;
use StudentLink\Jwt\JwtHelper;
...
use StudentLink\UpdateFailedException;
use StudentLink\ValidationException;
use StudentLink\AccessDeniedException;
use StudentLink\VersionMismatchException;
use StudentLink\EntityNotFoundException;
use StudentLink\DuplicateEntityException;
use StudentLink\FileNotFoundException;
use StudentLink\FileContentException;
...
abstract class AbstractApiControllerHelper extends AbstractRestfulController
{
    ...
    {
        $errorType = StudentLinkServiceException::ERR_TYPE_ERROR;
        $errorCode = StudentLinkServiceException::ERR_CODE_INTERNAL_SERVER_ERROR;
        $context = null;
        $message = null;
        if ($e instanceof ValidationException) {
            $errorType = StudentLinkServiceException::ERR_TYPE_ERROR;
            $errorCode = StudentLinkServiceException::ERR_CODE_NOT_ACCEPTABLE; //Input data is not acceptable.
            $message = 'Validation error(s).';
            $context = ['errors' => $e->getErrors()];
        } elseif ($e instanceof VersionMismatchException) {
            $errorType = StudentLinkServiceException::ERR_TYPE_ERROR;
            $errorCode = StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED; //Version mismatch is precondition failure.
            $message = 'Entity version sent by client does not match the version on server.';
        } elseif ($e instanceof StudentLinkServiceException) {
            ...
            $errorType = StudentLinkServiceException::ERR_TYPE_ERROR;
            $errorCode = StudentLinkServiceException::ERR_CODE_INTERNAL_SERVER_ERROR; //Unexpected error is always HTTP 500.
            $message = 'Unexpected error.';
        }
        if (StudentLinkServiceException::ERR_TYPE_FAILURE == $errorType) {
            $errorCode = StudentLinkServiceException::ERR_CODE_OK;
        }
