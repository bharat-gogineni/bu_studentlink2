...
use StudentLink\Workflow\WorkflowFactory;
...
class WorkflowInstanceServiceTest extends AbstractServiceTest
{
    ...
    protected function setUp(): void
    {
        $this->loadConfig();
        parent::setUp();
        $this->workflowInstanceService = $this->getApplicationServiceLocator()->get(\StudentLink\Service\WorkflowInstanceService::class);
    ...
    {
        if (enableCamunda == 0) {
            $mockProcessEngine = Mockery::mock('\StudentLink\Workflow\Camunda\ProcessEngineImpl');
            $workflowService = $this->getApplicationServiceLocator()->get(\StudentLink\Service\WorkflowInstanceService::class);
    ...
    public function testSubmitActivityWithTimeoutError()
    {
        ...
        if (enableCamunda == 0) {
            $mockActivityEngine = Mockery::mock('\StudentLink\Workflow\Camunda\ActivityImpl');
            $workflowService = $this->getApplicationServiceLocator()->get(\StudentLink\Service\WorkflowInstanceService::class);
            $req = new Request('GET', '/');
            $prev = new \Exception();
