...
use StudentLink\Security\SecurityManager;
...
use StudentLink\Service\AccountService;
...
class AnnouncementService extends AbstractService
{
    ...
    public function createAnnouncement(&$inputData, $params = null)
    {
        ...
        if (isset($data['type'])) {
            if (!($data['type'] == 'ANNOUNCEMENT' || $data['type'] == 'HOMESCREEN')) {
                throw new ServiceException("Announcement Type must be ANNOUNCEMENT or HOMESCREEN", 'invalid.type.given', StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED);
            }
        }
        ...
        try {
            ...
            if (count($result) > 0) {
                throw new ServiceException("Announcement already exists", "announcement.exists", StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED);
            }
    ...
    public function updateAnnouncement($uuid, &$inputData, $accountId = null)
    {
        ...
        if (is_null($obj)) {
            throw new ServiceException("Announcement not found", "announcement.not.found", StudentLinkServiceException::ERR_CODE_NOT_FOUND);
        }
        $originalArray = $obj->toArray();
        if (isset($accountId)) {
            if ($accountId != $originalArray['account_id']) {
                throw new ServiceException("Announcement does not belong to the account", "announcement.not.found", StudentLinkServiceException::ERR_CODE_NOT_FOUND);
            }
        }
        if (isset($data['type'])) {
            if (!($data['type'] == 'ANNOUNCEMENT' || $data['type'] == 'HOMESCREEN')) {
                throw new ServiceException("Announcement Type must be ANNOUNCEMENT or HOMESCREEN", 'invalid.type.given', StudentLinkServiceException::ERR_CODE_NOT_FOUND);
            }
        }
        $form = new Announcement();
    ...
    public function getAnnouncementsList($filterParams, $params)
    {
        ...
        if (isset($params['type'])) {
            ...
            throw new ServiceException("Announcement Type must be specified", 'type.is.required', StudentLinkServiceException::ERR_CODE_PRECONDITION_FAILED);
        }
        $where = "";
        $pageSize = 20;
        $offset = 0;
    ...
    public function saveTeam($params, $data)
    {
        ...
        if (isset($params['accountId'])) {
            ...
            throw new ServiceException("Account does not exist", "account.not.found", StudentLinkServiceException::ERR_CODE_NOT_FOUND);
        }
